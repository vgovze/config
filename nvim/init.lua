...
require("lazy").setup({
	...
	-- Use the `dependencies` key to specify the plugin dependencies.
	...
	{ -- Fuzzy Finder (files, lsp, etc)
		...
		dependencies = {
			...
			{ -- If encountering errors, see telescope-fzf-native README
				...
				-- `build` is used to run some command when the pluginis
				-- installed/updated, not every time Neovim starts up.
				build = "make",
	...
	{ -- LSP Configuration & Plugins
		...
		config = function()
			...
			--  with an lsp this function will be executed to configure the
			--  current buffer
			vim.api.nvim_create_autocmd("LspAttach", {
				group = vim.api.nvim_create_augroup("kickstart-lsp-attach", { clear = true }),
				callback = function(event)
					-- NOTE: Remember that Lua is a real programming language,
					-- and as such it is possible to define small helper and utility
					-- functions so you don't have to repeat yourself.
					...
					-- and you want to see the definition of its *type*,
					...
					-- a little while. See `:help CursorHold` for information about
			...
			--  - filetypes (table): Override the default list of
			--      associated filetypes for the server.
		...
		config = function() -- See `:help cmp`
			...
			cmp.setup({
				...
				mapping = cmp.mapping.preset.insert({
					...
					-- <c-l> will move you to the right of
					-- each of the expansion locations.
	...
	{ -- Highlight, edit, and navigate code
		...
		config = function(_, opts)
			...
			-- Prefer git instead of curl in order to improve connectivity
	...
	-- The following two comments only work if you have downloaded the
	-- kickstart repo, not just copy pasted the init.lua. If you want these
	-- files, they are in the repository, so you can just download them and
	-- place them in the correct locations.

	--  Here are some example plugins that I've included in
